...
public class RTDEBuffer {
    ...
    public static byte[] encode(Entry<String, String> entry, Object value) {
        ...
        if (type.equals("UINT32"))
            return Encoder.fromInt((Integer) value);

        if (type.equals("UINT64"))
            return Encoder.fromLong((Long) value);

        if (type.equals("INT32"))
            return Encoder.fromInt((Integer) value);

        if (type.equals("DOUBLE"))
            return Encoder.fromDouble((Double) value);
        ...
        if (type.equals("VECTOR3D")) {
            ...
            for (int i = 0; i < 3; i++) {
                System.arraycopy(Encoder.fromDouble(doubles[i]), 0, bytes, Double.BYTES * i, Double.BYTES);
            }
        ...
        if (type.equals("VECTOR6INT32")) {
            ...
            for (int i = 0; i < 6; i++) {
                System.arraycopy(Encoder.fromInt(ints[i]), 0, bytes, Integer.BYTES * i, Integer.BYTES);
            }
